...
case $B_CHOICE in
  1)
    PT_LINE="ClientTransportPlugin meek_lite,obfs2,obfs3,obfs4,scramblesuit,webtunnel exec /data/data/com.termux/files/usr/bin/lyrebird"
    BRIDGE_DATA=$(cat << 'EOF'
...
EOF
  ...
  2)
    PT_LINE="ClientTransportPlugin meek_lite,obfs2,obfs3,obfs4,scramblesuit,webtunnel exec /data/data/com.termux/files/usr/bin/lyrebird"
    BRIDGE_DATA="Bridge meek_lite 192.0.2.20:80 url=https://1603026938.rsc.cdn77.org front=www.phpmyadmin.net utls=HelloRandomizedALPN"
    ;;
  3)
    PT_LINE="ClientTransportPlugin snowflake exec /data/data/com.termux/files/usr/bin/lyrebird"
    BRIDGE_DATA=$(cat << 'EOF'
...
SocksPort \$PORT
DataDirectory \$DIR
$(if [ -n "$PT_LINE" ]; then echo "UseBridges 1"; echo "$PT_LINE"; echo "$BRIDGE_DATA"; else echo "UseBridges 0"; fi)
$PROXY_LINE
TORCFG
